...
from .coordinator import PhotoFrameCoordinator, ProcessingSettingEntityMixin
...
async def async_setup_entry(
    ...
    # entity and the timezone-offset number by the time-zone text entity; drop
    # the stale registry entries so upgrades don't leave permanently-unavailable
    # orphans behind.
    ent_reg = er.async_get(hass)
    for suffix in ("rotation_interval", "timezone_offset"):
        stale = ent_reg.async_get_entity_id("number", DOMAIN, f"{entry.entry_id}_{suffix}")
        if stale:
